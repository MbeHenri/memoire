...
#![Modif]! suppresion des affichages de la librairie bokeh
...
def create_bokeh(G, title, fname, extract_size=15, feature_size=10):
    ...
    def calc_alpha(x): return 0.1 + 0.6 * (x / max_weight)
    edge_attrs = {(s, e): calc_alpha(d['weight'])
    ...
    node_hover_tool = HoverTool(
        tooltips=[("Name", "@index"), ("Members", "@members")])
    plot.add_tools(node_hover_tool, WheelZoomTool(),
    ...
    graph_renderer.node_renderer.glyph = Circle(
        size='_size', fill_color="_color")
    graph_renderer.node_renderer.selection_glyph = Circle(
        size=15, fill_color="orange")  # works
    graph_renderer.node_renderer.hover_glyph = Circle(
        size=15, fill_color="red")  # works
    graph_renderer.edge_renderer.glyph = MultiLine(
        line_alpha="_alpha", line_width=1)
...
def Bioactivity_plot(MasterOutput, Network_In_Path, Bioactivity_Data_In, title='Bioactivity_Plot', fname='Bioactivity_Plot', Bioactivity_Low=0.33, Bioactivity_Med=0.66, Bioactivity_High=1.0):
    ...
    nx.write_graphml(Network_In, os.path.join(
        MasterOutput, str(title+'.graphml')))
    create_bokeh(Network_In, title, os.path.join(
        MasterOutput, fname+'.html'), extract_size=15, feature_size=10)
